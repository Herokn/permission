<template>
  <t-layout :class="[`${prefix}-layout`, props.className]">
    <t-tabs
      v-if="settingStore.isUseTabsRouter && !isHidePdfConfig && tabRouters.length > 0"
      drag-sort
      theme="card"
...
const handleChangeCurrentTab = (path: string) => {
  ...
  console.log('handleChangeCurrentTab-path:', path, tabRouters);
