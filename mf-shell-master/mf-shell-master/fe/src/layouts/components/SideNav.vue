    ...
    <t-menu :class="menuCls" :theme="theme" :value="active" :collapsed="collapsed" :default-expanded="defaultExpanded">
      <template #logo>
        <span v-if="showLogo" :class="`${prefix}-side-nav-logo-wrapper`" @click="goHome">
          <img :src="getLogo()" :class="`${prefix}-side-nav-logo-${collapsed ? 't' : 'tdesign'}-logo`" />
        </span>
        <span>Wan Bridge</span>
      </template>
...
import AssetLogoFull from '@/assets/imgs/header/logo.png';
import AssetLogo from '@/assets/imgs/header/logo.png';
...
.tdesign-starter-side-nav-logo-wrapper{
  ...
  :deep(.tdesign-starter-side-nav-logo-tdesign-logo){
    // width:auto
  }
</style>
