<template>
  <router-view v-if="!isRefreshing" :key="$route.fullPath" v-slot="{ Component }">
...
import { useTabsRouterStore } from '@/stores';
...
const aliveViews = computed(() => {
  ...
  return tabRouters
    .filter((route) => {
      ...
      // console.log(route, keepAliveConfig, isRouteKeepAlive, 'isRouteKeepAlive');
...
const isRefreshing = computed(() => {
  const tabsRouterStore = useTabsRouterStore();
  const { isRefreshing } = tabsRouterStore;
  return isRefreshing;
});
</script>
